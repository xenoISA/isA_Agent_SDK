    ...
    async def _load_skills(self, skill_names: List[str], project_root: Optional[str] = None) -> Dict[str, str]:
        ...
        for skill_name in skill_names:
            ...
            try:
                ...
                # MCP client returns {"result": {...}, "context": ...}
                # The actual contents are in resource["result"]["contents"]
                result = resource.get("result", resource) if resource else None
    ...
    async def get_runtime_context(
        ...
        if skills:
            ...
            plan_tool_names = [
                ...
                "replan_execution",
                "adjust_plan",  # Fixed: was update_plan_step (doesn't exist)
                "get_plan_status"
            ]
            ...
            self.logger.info(
                f"[PHASE:CONTEXT] plan_tools_added | "
                f"user_id={user_id} | "
                f"plan_tools={plan_tool_names} | "
