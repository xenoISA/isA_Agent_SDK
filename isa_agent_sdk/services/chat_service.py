...
class ChatService(BaseService):
    ...
    async def _execute_desktop_mode(
        ...
        """
        Execute chat in desktop mode - delegate tool execution to user's desktop.

        This mode:
        1. Finds the user's online desktop via Pool Manager
        2. Processes the query locally but routes tool calls to the desktop
        3. Returns results from the desktop agent

        Flow: isA Agent -> Pool Manager -> Desktop Agent -> Local Filesystem
        """
        ...
        try:
            ...
            yield EventEmitter.system_info(
                ...
                event_type="desktop_connecting"
            ).to_dict()

            desktop_id = await desktop_client.acquire_desktop(user_id)

            if not desktop_id:
                yield EventEmitter.system_error(
                    session_id,
                    f"No online desktop found for user {user_id}. Please ensure your desktop agent is running.",
                    error_type="NoDesktopFound"
                ).to_dict()
                return

            yield EventEmitter.system_info(
                session_id,
                f"Connected to desktop",
                event_type="desktop_connected",
                desktop_id=desktop_id
            ).to_dict()

            # For desktop mode, we need to analyze the request and execute appropriate tools
            # This is a simplified implementation - it directly executes based on keywords
            # A full implementation would use the LangGraph to reason about which tools to use

            # Simple command parsing for common operations
            user_input_lower = user_input.lower()

            if any(keyword in user_input_lower for keyword in ["list", "ls", "show files", "what files", "directory"]):
                # Extract path from input or use default
                import re
                path_match = re.search(r'(?:in|at|from|of)\s+([/~][^\s]+)', user_input)
                path = path_match.group(1) if path_match else "."

                yield EventEmitter.tool_call(
                    session_id,
                    tool_name="ls_directory",
                    tool_args={"path": path},
                    tool_id="desktop_ls_1"
                ).to_dict()

                result = await desktop_client.execute_tool("ls_directory", {"path": path})
                ...
                if result.get("status") == "success":
                    ...
                    response = f"Found {len(entries)} items in {path}:\n\n"
                    for entry in entries:
                        entry_type = "📁" if entry.get("type") == "directory" else "📄"
                        size = f" ({entry.get('size', 0)} bytes)" if entry.get("type") == "file" else ""
                        response += f"{entry_type} {entry.get('name')}{size}\n"
                else:
                    response = f"Error listing directory: {result.get('error', 'Unknown error')}"

            elif any(keyword in user_input_lower for keyword in ["create", "write", "make a file", "new file"]):
                # Extract file path and content
                import re
                # Try to find file path
                path_match = re.search(r'(?:called|named|at|to)\s+([/~][^\s]+\.?\w*)', user_input)
                if not path_match:
                    path_match = re.search(r'([/~][^\s]+\.md)', user_input)

                if path_match:
                    file_path = path_match.group(1)
                    # Try to extract content description
                    content = f"# File created via Desktop Agent\n\nThis file was created through the isA Agent SDK desktop execution flow.\n\nCreated at: {datetime.now().isoformat()}\nUser: {user_id}\nDesktop: {desktop_id}\n"

                    yield EventEmitter.tool_call(
                        session_id,
                        tool_name="write_file",
                        tool_args={"file_path": file_path, "content": content},
                        tool_id="desktop_write_1"
                    ).to_dict()

                    result = await desktop_client.execute_tool("write_file", {
                        "file_path": file_path,
                        "content": content
                    })
                    ...
                    if result.get("status") == "success":
                        response = f"Successfully created file: {file_path}\nBytes written: {result.get('data', {}).get('bytes_written', 0)}"
                    else:
                        response = f"Error creating file: {result.get('error', 'Unknown error')}"
                else:
                    response = "Please specify a file path. Example: 'create a file at /path/to/file.md'"

            elif any(keyword in user_input_lower for keyword in ["read", "show content", "cat", "open file"]):
                # Extract file path
                import re
                ...
                if path_match:
                    ...
                    yield EventEmitter.tool_call(
                        ...
                        tool_id="desktop_read_1"
                    ).to_dict()

                    result = await desktop_client.execute_tool("read_file", {"file_path": file_path})
                    ...
                    if result.get("status") == "success":
                        ...
                        response = f"Error reading file: {result.get('error', 'Unknown error')}"
                else:
                    response = "Please specify a file path. Example: 'read /path/to/file.txt'"

            elif any(keyword in user_input_lower for keyword in ["run", "execute", "bash", "command"]):
                # Extract command
                import re
                cmd_match = re.search(r'(?:run|execute|command)\s*[:\-]?\s*[`"]?([^`"]+)[`"]?', user_input, re.IGNORECASE)
                ...
                if cmd_match:
                    ...
                    yield EventEmitter.tool_call(
                        ...
                        tool_id="desktop_bash_1"
                    ).to_dict()

                    result = await desktop_client.execute_tool("bash_execute", {"command": command})
                    ...
                    if result.get("status") == "success":
                        ...
                        response = f"Command executed (exit code: {exit_code})\n\n"
                        if stdout:
                            response += f"Output:\n```\n{stdout}\n```\n"
                        if stderr:
                            response += f"Errors:\n```\n{stderr}\n```"
                    else:
                        response = f"Error executing command: {result.get('error', 'Unknown error')}"
                else:
                    response = "Please specify a command. Example: 'run command: ls -la'"
            else:
                # Default: provide help
                response = (
                    "I'm connected to your desktop and can help with:\n\n"
                    "- **List files**: 'list files in /path/to/dir'\n"
                    "- **Create files**: 'create a file at /path/to/file.md'\n"
                    "- **Read files**: 'read /path/to/file.txt'\n"
                    "- **Run commands**: 'run command: your_command_here'\n\n"
                    f"Desktop: {desktop_id}\n"
                )

            # Stream the response
            for token in response:
                yield EventEmitter.content_token(session_id, token).to_dict()
            ...
            yield EventEmitter.system_error(
                session_id,
                f"Desktop execution error: {str(e)}",
                error_type=type(e).__name__
            ).to_dict()
    ...
    async def execute(
        ...
        env: Optional[str] = None,  # Execution environment: desktop, cloud_pool, cloud_shared
    ) -> AsyncIterator[Dict[str, Any]]:
        ...
        try:
            ...
            # Check for desktop execution environment
            if env == "desktop":
                self.logger.info(
                    f"[PHASE:SERVICE] desktop_mode_detected | "
                    f"session_id={session_id} | "
                    f"user_id={user_id} | "
                    f"routing to user desktop via pool_manager"
                )
                async for event in self._execute_desktop_mode(
                    user_input=user_input,
                    session_id=session_id,
                    user_id=user_id,
                    auth_token=auth_token
