...
class AgentExecutorNode(BaseNode):
    ...
    async def _execute_single_task(
        ...
        try:
            ...
            # Build execution prompt using MCP (same pattern as other nodes)
            # Get user_instructions from system_prompt_config
            system_prompt_config = config.get("configurable", {}).get("system_prompt_config")
            user_instructions = ""
            if system_prompt_config and hasattr(system_prompt_config, 'append') and system_prompt_config.append:
                user_instructions = system_prompt_config.append

            # Format tools for prompt
            tools_str = ", ".join([t.get("name", str(t)) if isinstance(t, dict) else str(t) for t in tools]) if tools else ""
            ...
            execution_prompt = await self.mcp_get_prompt("task_execution_prompt", prompt_args, config)

            # Fallback if MCP prompt not available
            if not execution_prompt:
                execution_prompt = f"""Execute the following task step by step.
...
Provide detailed feedback on your progress and results."""
