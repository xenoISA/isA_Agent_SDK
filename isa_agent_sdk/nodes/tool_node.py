...
class ToolNode(BaseNode):
    ...
    async def _execute_logic(self, state: AgentState, config: RunnableConfig) -> AgentState:
        ...
        auto_approve_plans = state.get("auto_approve_plans", False)
        if auto_approve_plans:
            self.hil_router.set_auto_approve_plans(True)
            self.logger.warning("⚠️ [ToolNode] Auto-approve plans ENABLED via state")
        ...
        # Check for autonomous planning activation
        # When create_execution_plan succeeds, route to AgentExecutorNode
        autonomous_plan = self._detect_autonomous_plan(tool_info_list, tool_messages)
        ...
        # Activate autonomous execution if plan was created
        if autonomous_plan:
            result.update({
                "is_autonomous": True,
                "execution_strategy": "autonomous_planning",
                "execution_plan": autonomous_plan.get("plan", {}),
                "autonomous_tasks": autonomous_plan.get("tasks", []),
                "task_list": autonomous_plan.get("tasks", []),  # AgentExecutorNode uses this
            ...
            self.logger.info(
                ...
                f"task_count={len(autonomous_plan.get('tasks', []))} | "
                f"routing_to=agent_executor"
            )
        # Override next action if already set by plan_tool (legacy support)
        elif "next_action" in state and state["next_action"] == "agent_executor":
    ...
    def _detect_autonomous_plan(
        self,
        tool_info_list: List[Tuple[str, dict, str]],
        tool_messages: List[ToolMessage]
    ) -> Optional[Dict[str, Any]]:
        """
        Detect if create_execution_plan was called and extract the plan.

        This enables the flow: ReasonNode → ToolNode → AgentExecutorNode
        by setting up autonomous execution when a plan is successfully created.

        Args:
            tool_info_list: List of (tool_name, tool_args, tool_call_id) tuples
            tool_messages: List of ToolMessage responses
        ...
        try:
            ...
            if not plan_data:
                self.logger.warning(f"[AUTONOMOUS] Failed to parse plan response: {content[:200]}")
                return None

            # Check for success status
            status = plan_data.get('status', '')
            if status == 'error' or 'error' in content.lower()[:100]:
                self.logger.warning(f"[AUTONOMOUS] Plan creation failed: {plan_data.get('message', 'unknown error')}")
                return None

            # Extract tasks from various possible structures
            tasks = None
            plan_info = None

            # Format 1: Auto-approve response: {"approved": true, "content": {"tasks": [...], "plan_id": "..."}}
            if plan_data.get('auto_approved') and 'content' in plan_data:
                content_data = plan_data['content']
                if isinstance(content_data, dict):
                    tasks = content_data.get('tasks', [])
                    plan_id = content_data.get('plan_id', 'unknown')
                    plan_info = content_data
                    self.logger.info(f"[AUTONOMOUS] Detected auto-approved plan: {plan_id}")

            # Format 2: MCP success response: {"status": "success", "data": {"tasks": [...], "plan_id": "..."}}
            elif 'data' in plan_data and isinstance(plan_data['data'], dict):
                tasks = plan_data['data'].get('tasks', [])
                plan_id = plan_data['data'].get('plan_id', 'unknown')
                plan_info = plan_data['data']

            # Format 3: Direct plan structure: {"tasks": [...], "plan_id": "..."}
                ...
                self.logger.warning(f"[AUTONOMOUS] No tasks found in plan response: {list(plan_data.keys())}")
                return None

            if not tasks:
                self.logger.warning("[AUTONOMOUS] Empty task list in plan")
                return None

            self.logger.info(
                f"[AUTONOMOUS] Plan detected | "
                f"plan_id={plan_id} | "
