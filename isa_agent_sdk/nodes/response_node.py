...
class ResponseNode(BaseNode):
    ...
    async def _execute_logic(self, state: AgentState, config: RunnableConfig) -> AgentState:
        ...
        # Get user_instructions from system_prompt_config (if available)
        system_prompt_config = config.get("configurable", {}).get("system_prompt_config") if config else None
        user_instructions = ""
        if system_prompt_config:
            # Check for full replacement first
            if hasattr(system_prompt_config, 'is_replacement') and system_prompt_config.is_replacement:
                # Full replacement - use replace as the entire prompt
                response_prompt = system_prompt_config.replace
                self.logger.info(
                    f"[PHASE:NODE_RESPONSE] using_replacement_prompt | "
                    f"session_id={session_id} | "
                    f"length={len(response_prompt)}"
                )
            elif hasattr(system_prompt_config, 'append') and system_prompt_config.append:
                user_instructions = system_prompt_config.append

        # Only fetch from MCP if not using full replacement
        if not (system_prompt_config and hasattr(system_prompt_config, 'is_replacement') and system_prompt_config.is_replacement):
            if output_format == "json":
                ...
                if not response_prompt:
                    ...
                    default_prompt = await self.mcp_get_prompt("default_response_prompt", {"conversation_summary": conversation_summary, "user_instructions": user_instructions}, config)
                    if default_prompt:
                ...
                if not response_prompt:
                    # Fallback to simple response prompt with user instructions
                    fallback = f"Please provide a helpful response based on this conversation:\n\n{conversation_summary}"
                    if user_instructions:
                        fallback += f"\n\nAdditional Instructions:\n{user_instructions}"
                    response_prompt = fallback
